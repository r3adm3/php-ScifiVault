...
<table width="75%" border="1" align="center">
	...
	<tr> <td> Date </td> <td> Item </td> <td> Qty </td> <td> Comments </td> <td> Cancel? </td> </tr>

<?php

//query to get all items in basket
$strQuery = "select t.Date, t.qty, t.stockID, c.name, t.emailaddress, t.comment from tbl_PreOrder t inner join tblItem c on t.stockID = c.stockID where t.emailaddress = '" . $strEmailAddress . "'";
...
while ($line = mysql_fetch_array($strResult, MYSQL_ASSOC))
{
	...
	echo "\n\t<tr> <td> " . date('jS M Y', $timestamp) . " </td> \n\t<td><a href='/displayItem.php?Item=" . $line["stockID"]  ."'>" . $line["name"] . "</a></td>\n\t<td> " . $line["qty"] . " </td>\n\t<td> " . $line["comment"] . "</td>";

	echo "<td align='center'><form action='removePreOrder.php' method='post'><input type='hidden' name='emailaddress' value='" . $line["emailaddress"] . "'><input type='hidden' name='altRemove' value='-'><input type='hidden' name='page' value='" . $_SERVER['REQUEST_URI']. "'><input type='hidden' name='removeitem' value='" . $line["stockID"] . "'><INPUT TYPE='image' SRC='/images/buttons/trash.gif' ALT='Remove' name='remove' value='-'></form>";
