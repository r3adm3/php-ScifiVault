...
if (isset($_POST['Buy']) or isset($_POST['altBuy'])) {
	...
	while ($line = mysql_fetch_array($strLockResult, MYSQL_ASSOC))
	{
		if ($line["ColumnLock"] == 'YES')
		{
			...
			funcLogToDebug("addToBasket2.php: Item Locked in database");
			exit();
	...
	while ($line = mysql_fetch_array($strStockResult, MYSQL_ASSOC))
	{
		...
		if ($line["NoOfItems"] == 0)
		{
			//funcDebug ("Fired of an email");
			mail("adrian@nofishhere.com,james@scifivault.com,david@scifivault.com,hilary@scifivault.com", "ScifiVault Stock Warning (" . $itemcode . "x" . $qty . ")"  , " \n Stock is at 0 for  " . $itemcode ,
     "From: webmaster@{$_SERVER['SERVER_NAME']}\r\n" .
     "Reply-To: webmaster@{$_SERVER['SERVER_NAME']}\r\n" .
     "X-Mailer: PHP/" . phpversion());

		}
		elseif ($line["NoOfItems"] < 3)
		{
			//funcDebug ("Fired of an email");
			mail("adrian@nofishhere.com,james@scifivault.com,david@scifivault.com,hilary@scifivault.com", "ScifiVault Stock Warning (" . $itemcode . "x" . $qty . ")"  , " \n Stock is at " . ($line["NoOfItems"] - $qty) ." for  " . $itemcode ,
     "From: webmaster@{$_SERVER['SERVER_NAME']}\r\n" .
     "Reply-To: webmaster@{$_SERVER['SERVER_NAME']}\r\n" .
     "X-Mailer: PHP/" . phpversion());
		}
