...
//break up the results sets into a number of pages ***************************************************************
		...
		if ($strPages <> 0)
		{
			...
			echo "<a href='search.php?Search=" . $strSearch . "&p=" . $strPrevious . "'> &lt;&lt;Prev</a> ";}
			...
			for ($counter=0;$counter<$strPages;$counter+=1)
			{
				...
				echo "  <a href='search.php?Search=" . $strSearch . "&p=" . $counter . "'>" . $displayCounter . "</a>" ;
...

{
			...
			echo "\t<TR width='100%' valign='center'>    <TD width='50'> <img src='". $row["smallPicture"] . "'> </TD>"
				...
				if ($row["RRP"] == $row["SaleRRP"] or $row["SaleRRP"]==0.00)
				{
							echo "&pound;" . $row["RRP"];
				}
				else
