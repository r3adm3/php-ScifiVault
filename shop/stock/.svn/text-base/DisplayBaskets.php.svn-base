...
while ($line = mysql_fetch_array($strResult, MYSQL_ASSOC))
{
	...
	echo "<tr>\n\t<td>" . $line["AutoNumber"] . "</td><td><a href='basketAdmin.php?BasketID=" . $line["PHPSessionID"] . "'>" . $line["PHPSessionID"] . "<a></td><td> " . $line["timeStmp"] ." </td> <td><input type='radio' name='ToDelete' value='" . $line["PHPSessionID"] . "'></td>";
