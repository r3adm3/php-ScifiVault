...
";
      ...
      <br>
      <a href="http://shop.scifivault.com/Category.php?sTag=ST"><img src="images/STARTREKbanner.jpg" width="200" height="100" border="0"></a><a href="http://shop.scifivault.com/subCategory.php?sTag=ALN&vTag=ALL"><img src="images/alienbanner.jpg" width="200" height="100" border="0"></a></td>
...
//Title stuff. **************************************************************************************
		...
		//if ($strVTag == "ALL") { $strVTagEx = ""; } else {$strVTagEx = $strVTag;}
		...
		while ($lineTitle = mysql_fetch_array($strResultTitle, MYSQL_ASSOC))
		{
			...
			$strCat = $lineTitle["colTitle"];

		}

		$strQueryTitle4 = "SELECT colTitle FROM tbl_Synopsis where colCategory='" . $strSTag . "' and colSubCategory =''";

		$strResultTitle4 = mysql_query($strQueryTitle4) or die ("Query Failed :" . mysql_error());

		While  ($lineCat = mysql_fetch_array($strResultTitle4, MYSQL_ASSOC))
		{

			$strUpCat = $lineCat["colTitle"];
		...
		if ($strVTag == "ALL") {} else {
		echo "\n<A href='Category.php?sTag=" . $strSTag . "'>" . $strUpCat . "</a>";
		}

		echo "<br><H1>" . $strCat . "</H1>";

		//****************************************************************************************************
/*/Category menu ************************************************************************************

		//$strQuery = "SELECT distinct Version, VersionTag FROM `tblItem` where SubjectTag='" . $strSTag. "'";
$strQuery = "SELECT distinct Version, VersionTag, count(NoOfItems) as cntItems FROM `tblItem` where SubjectTag='" . $strSTag. "' group by Version having  (count(NoOfItems) > 0 )";
		...
		$strResult = mysql_query($strQuery) or die ("Query Failed :" . mysql_error());
			...
			while ($line2 = mysql_fetch_array($strResult, MYSQL_ASSOC))
			{
				...
				echo "   <a href='subCategory.php?sTag=" . $strSTag . "&vTag=" . $line2["VersionTag"]. "'><img src='/images/buttons/" . $line2["VersionTag"] . ".jpg' border=0></a> ";
...
//subCategory menu ************************************************************************************
		...
		//$strQuery3 = "SELECT distinct Category, CategoryTag FROM `tblItem` where SubjectTag='" . $strSTag. "'";
	$strQuery3 = "SELECT distinct Category, CategoryTag, NoOfItems FROM `tblItem` where SubjectTag='" . $strSTag. "' and VersionTag='" . $strVTag . "' group by Category having (NoOfItems>0) order by CategoryTag";
		...
		while ($line3 = mysql_fetch_array($strResult3, MYSQL_ASSOC))
		{

			echo "   <a href='subCategory2.php?sTag=" . $strSTag . "&vTag=" . $strVTag . "&cTag=" . $line3["CategoryTag"]. "'> <img border=0 src='/images/buttons/" . $line3["Category"] . ".gif'></a> ";
...
?>
      ...
      <table id="partial" width="100%"  border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#002A54">
		...
		$strQuery = "SELECT stockID FROM tblItem where SubjectTag='" . $strSTag . "' and VersionTag='" . $strVTag . "' and DisplayonSubCatPage='1' and NoOfItems <>'0'";

		$strResult = mysql_query($strQuery) or die ("Query Failed :" . mysql_error());

		//$arrFrontStock = mysql_query($strQuery) or die ("Query Failed :" . mysql_error());
		
		funcDebug ("Result here: " . count ($arrFrontStock));
		...
		if (count ($arrFrontStock)==0)
		{
				...
				$strQuery = "SELECT stockID FROM tblItem where SubjectTag='" . $strSTag . "' and VersionTag='" . $strVTag . "' and NoOfItems >'0' LIMIT 3";
		...
		//print_r($arrFrontStock);

		$numElements = count ($arrFrontStock)+1;

		funcDebug ($numElements);

		for ($counter=1; $counter < $numElements; $counter++)
		{
			$strQuery4 = "SELECT stockID, smallPicture, Name, ShortDescription, NoOfItems, RRP, SaleRRP FROM tblItem where stockID ='" . $arrFrontStock[$counter-1] . "'";

			$strResult4 = mysql_query($strQuery4) or die ("Query Failed :" . mysql_error());

			funcDebug("Before While Loop:" . $arrFrontStock[$counter-1]);

			while ($line2 = mysql_fetch_array($strResult4, MYSQL_ASSOC))
			{

				funcDebug ("In While Loop: " . $line2["stockID"]);
...
?>
      ...
      //break up the results sets into a number of pages ***************************************************************
	  		...
	  		$strNumberQuery = "select count(*) as ItemCount from tblItem where SubjectTag='" . $strSTag . "' and VersionTag='" . $strVTag . "' and NoOfItems <> -1";
	  		...
	  		if ($strPages <> 0)
	  		{
	  			...
	  			echo "<a href='subCategory.php?sTag=" . $strSTag . "&vTag=" . $strVTag . "&p=" . $strPrevious  . "'> &lt;&lt; Prev</a> ";}
...
?>
	...
	?>
      ...
      <p><a href="http://shop.scifivault.com/subCategory.php?sTag=WOW&vTag=ALL"><img src="images/WOFWARbanner.jpg" width="200" height="100" border="0"></a><a href="http://shop.scifivault.com/subCategory.php?sTag=EVE&vTag=ALL"><img src="images/EVEbanner.jpg" width="200" height="100" border="0"></a><a href="http://shop.scifivault.com/subCategory.php?sTag=JB&vTag=ALL"><img src="images/007banner.jpg" width="200" height="100" border="0"></a><a href="http://shop.scifivault.com/subCategory.php?sTag=DW&vTag=ALL"><img src="images/DRWHObanner.jpg" width="200" height="100" border="0"></a><a href="http://shop.scifivault.com/Category.php?sTag=SW"><img src="images/STARWARSbanner.jpg" width="200" height="100" border="0"></a><a href="http://shop.scifivault.com/subCategory.php?sTag=SG&vTag=ALL"><img src="images/STARGATEbanner.jpg" width="200" height="100" border="0"></a></p>
    </td>
  </tr>
</table>
<br>
