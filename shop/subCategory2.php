...
//Title stuff. **************************************************************************************
		...
		$strQueryTitle = "SELECT * FROM tbl_Synopsis where colCategory='" . $strSTag . "' and colSubCategory='" . $strVTagEx . "'";
		...
		while ($lineTitle = mysql_fetch_array($strResultTitle, MYSQL_ASSOC))
		{
			...
			$strQuerySubCat = "SELECT Category from tblItem where subjectTag= '" . $strSTag . "' and categoryTag='" . $strCTag . "'";

			$strResultSubCat = mysql_query($strQuerySubCat) or die ("Query Failed :" . mysql_error());

			while ($lineSubCat = mysql_fetch_array($strResultSubCat, MYSQL_ASSOC))
			{
				$strCat = $lineSubCat["Category"];
			}
			...
			echo "<a href='subCategory.php?sTag=" .$strSTag . "&vTag=" . $strVTag . "'>" . $lineTitle["colTitle"] . "</a>";
...
/*/subCategory menu ************************************************************************************
		...
		$strQuery3 = "SELECT distinct Category, CategoryTag, count(NoOfItems) FROM `tblItem` where SubjectTag='" . $strSTag. "' and VersionTag='" . $strVTag . "' and NoOfItems <> 0 group by Category having (count(NoOfItems)>0)";
...
//break up the results sets into a number of pages ***************************************************************
		...
		$strNumberQuery = "select count(*) as ItemCount from tblItem where SubjectTag='" . $strSTag . "' and VersionTag='" . $strVTag . "' and CategoryTag = '" . $strCTag . "' and NoOfItems <> 0";
		...
		if ($strPages <> 0)
		{
			...
			echo "<a href='subCategory2.php?sTag=" . $strSTag . "&vTag=" . $strVTag . "&cTag=" . $strCTag . "&p=" . $strPrevious  . "'> &lt;&lt; Prev</a> ";}
			...
			for ($counter=0;$counter<$strPages;$counter+=1)
			{
				...
				echo "<a href='subCategory2.php?sTag=" . $strSTag . "&vTag=" . $strVTag . "&cTag=" . $strCTag . "&p=" . $counter . "'>" . $displayCounter . "</a>" ;
			...
			{$strNext = $strPTag + 1;
			echo "<a href='subCategory2.php?sTag=" . $strSTag . "&vTag=" . $strVTag . "&cTag=" . $strCTag . "&p=" . $strNext  . "'> Next &gt;&gt; </a>";}
...
//end of break up the results sets into a number of pages ***************************************************************
		...
		$strQueryDisp = "SELECT stockID, smallPicture, Name, ShortDescription, NoOfItems, RRP, SaleRRP FROM tblItem where SubjectTag='" . $strSTag . "' and VersionTag='" . $strVTag . "' and CategoryTag = '" . $strCTag . "' and NoOfItems <> -1  order by Name LIMIT " . $strPTag * 5 . ", 5";
		...
		//$strQueryDisp = "SELECT stockID, smallPicture, Name, ShortDescription, NoOfItems, RRP, SaleRRP FROM tblItem where SubjectTag='" . $strSTag . "' and VersionTag='" . $strVTag . "' and CategoryTag = '" . $strCTag . "'  order by Name LIMIT " . $strPTag * 5 . ", 5";
		...
		while ($lineDisp = mysql_fetch_array($strResultDisp, MYSQL_ASSOC))
		{
			...
			echo "\t<TR width='100%' valign='center'>    <TD width='50'> <a href='displayItem.php?Item=" . $lineDisp["stockID"] . "'><img src='". $lineDisp["smallPicture"] . "' border='0'></a> </TD>"
				...
				/*
				echo " </TD>"
				.	"\n<TD width='60'> <form action='addToBasket2.php' method='post'>  <br> <input TYPE='image' SRC='images/buttons/BUYBUTTON.gif' name='Buy'><input type='hidden' name='altBuy' value='Buy'> <input type='hidden' name='qty' value='1'><input type='hidden' name='item' value='". $lineDisp["stockID"] ."'> <input type='hidden' name='page' value='" . $_SERVER['REQUEST_URI']. "'></form></TD>"
				. "</TR>\n";
				*/
